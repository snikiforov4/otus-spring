...
class BookCommandsTest {
    ...
    class AddBook {
        ...
        void shouldNotCallGenreRepositoryIfGenreIdParamIsNotPositive() {
            ...
            verify(genreRepository, never()).findById(anyString());
            verify(bookRepository, times(1))
    ...
    private static Book createBookStub(@SuppressWarnings("SameParameterValue") String bookId) {
        Book book = new Book("Outsider");
        book.setId(bookId);
        return book;
    }

    private static Author createAuthorStub(@SuppressWarnings("SameParameterValue") String authorId) {
